...
def detect(save_txt=False, save_img=False):
    ...
    webcam = source == '0' or source.startswith('rtsp') or source.startswith('http') or source.endswith('.txt') or source.startswith('v4l2src')
...
#                    im0 = cv2.cvtColor(im0, cv2.COLOR_BGR2RGB)
    ...
    ######################################
                        ...
                        lb = test_LPRNet.test(crop_img, device)
...
#            cv2.circle(im0, (centroid[0], centroid[1]), 4, (0, 255, 0), -1)
            if lb is not None:
                cv2.putText(im0, lb, (30, 200), font, 1.5, (0,255,0), 5, cv2.FONT_HERSHEY_SIMPLEX,)
